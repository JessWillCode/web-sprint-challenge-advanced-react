...
The main differences between a class-based and functional component is that a functional component is just a javascript function that accepts props and returns a react element without a render function. A class based component is extended from React and creates a render function that returns a React element.

2. When does a componentWillMount function be called? What about a componentDidUpdate?

A componentWillMount is called just before a component is mounted on the DOM or when the render method is called. After this, the component gets mounted. If a componentDidMount function is in place, it will be called after this.

A componentDidUpdate is called after the render method of the component is finished executing. Unless stated in the function, this will happen any time there is a change.
...
1. Arrange- Setting up the react component we are testing

1. Act- executing behavior that we need to replicate if needed

1. Assert- pulling the response and checking that it is what we expect to happen.
